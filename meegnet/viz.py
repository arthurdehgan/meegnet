...
def compute_saliency_maps(dataset, net, sal_path, threshold=0.95, labels=None, epoched=False):
	...
	for j, sal_type in enumerate(('pos', 'neg')):
		if epoched:
			for i, label in enumerate(labels):
				sal_filepath = os.path.join(sal_path, f'{sub}_{labels[i]}_{sal_type}_sal_{threshold}confidence.npy')
				np.save(sal_filepath, np.array(target_saliencies[i][j]))
		else:
			lab = '' if not epoched else f'_{labels[label]}'
			sal_filepath = os.path.join(sal_path, f'{sub}{lab}_{sal_type}_sal_{threshold}confidence.npy')
...
def compute_saliency_based_psd(saliency, trial, w_size, fs):
	...
	for i, chan in enumerate(saliency):
		...
		transformed_data = Parallel(n_jobs=1)(
			delayed(compute_single_sal_psd)(index, trial[i, j], w_size, fs) for j, index in enumerate(windows_idx)
		)
...
def make_gif(image_list, output=None, duration=100, loop=0):
	...
	frame_one = frames[0]
	frame_one.save(output, format='GIF', append_images=frames, save_all=True, duration=duration, loop=loop)
...
def generate_saliency_figure(
	...
	assert topomap in ('timing', 'average', 'window'), (
	...
	for i, label in enumerate(saliencies.keys()):
		...
		for j, sensor_type in zip(range(0, n_blocs * block_size, block_size), sensors):
			...
			plt.imshow(grads, interpolation='nearest', aspect=1, vmin=-vlim, vmax=vlim, cmap=cmap)
			...
			if isinstance(info, str):
				linfo = np.load(
					os.path.join(info, f'camcan_info_{sensor_type.lower()}.npy'), allow_pickle=True
				).tolist()
			else:
				linfo = info
			im, _ = plot_topomap(
...
def plot_masked_epoch(data, mask, c1='white', c2='green', alpha=0.3, title: str = None):
	...
	assert dim == 2, f'Data is supposed to be a 2 dimension array, a {dim}-dimension array was given.'

	mask = (mask - mask.min()) / np.ptp(mask)
	fig, axes = plot_epoch(data, title)
	for i, ax in enumerate(axes):
		for j, x in enumerate(range(data.shape[1])):
			ax.axvline(x + 1, color=colorFader(c1, c2, mask[i, j]), zorder=0, alpha=alpha, linewidth=50)
...
def compute_cams(net, target_layers, dataset, verbose=3):
	...
	for sub in dataset.subject_list:
		...
		for i, _ in enumerate(sub_dataset.target_labels):
			...
			if high_conf_indices.sum() > 0:  # Only proceed if there are high-confidence predictions
