...
def create_net(net_option: str, input_size: int, n_outputs: int, net_params: dict = None) -> nn.Module:
	"""
	...
	if net_params is None:
		net_params = {'linear': 100, 'hlayers': 2, 'dropout': 0.5}
...
class EEGNetv4(CustomNet):
	def __init__(
		...
		self.feature_extraction = nn.Sequential(
			...
			nn.Conv2d(input_size[0], F1, (1, kernel_length), stride=1, bias=False, padding=(0, kernel_length // 2)),
			nn.BatchNorm2d(self.F1, momentum=batch_norm_momentum, affine=batch_norm_affine, eps=batch_norm_eps),
			...
			nn.BatchNorm2d(
				self.F1 * self.D, momentum=batch_norm_momentum, affine=batch_norm_affine, eps=batch_norm_eps
			),
			activation(),
			pool_class[pool_mode](kernel_size=(1, pool1_kernel_size), stride=(1, pool1_stride_size)),
			...
			nn.BatchNorm2d(self.F2, momentum=batch_norm_momentum, affine=batch_norm_affine, eps=batch_norm_eps),
			activation(),
			pool_class[pool_mode](kernel_size=(1, pool2_kernel_size), stride=(1, pool2_stride_size)),
			nn.Dropout(p=drop_prob),
		)
		...
		self.classif = nn.Sequential(
			nn.Conv2d(self.F2, n_outputs, (n_out_virtual_chans, self.final_conv_length), bias=True)
...
class EEGNet(CustomNet):
	...
	def __init__(
		...
		if dropout_option not in ['SpatialDropout2D', 'Dropout']:
			raise ValueError('dropout_option must be one of SpatialDropout2D or Dropout, passed as a string.')
		dropout_type = nn.Dropout2d if dropout_option == 'SpatialDropout2D' else nn.Dropout

		n_channels = input_size[1]
		self.feature_extraction = nn.Sequential(
			nn.Conv2d(input_size[0], n_filters, (1, filter_size), padding=(1, filter_size // 2), bias=False),
			nn.BatchNorm2d(n_filters),
			DepthwiseConv2d(
				n_filters, (n_channels, 1), depthwise_multiplier=depthwise_multiplier, padding=0, bias=False
			),
			...
			SeparableConv2d(
				n_filters * depthwise_multiplier, n_filters * depthwise_multiplier, (1, 16), padding=(1, 8), bias=False
			),
			nn.BatchNorm2d(n_filters * depthwise_multiplier),
			nn.ELU(),
			nn.AvgPool2d((1, 8)),
...
class MLP(CustomNet):
	...
	def __init__(self, input_size: tuple, n_outputs: int, hparams: dict) -> None:
		...
		self.feature_extraction = nn.Sequential(
			...
			*[
				nn.Sequential(
					nn.Linear(hparams['mlp_width'], hparams['mlp_width']), nn.Dropout(hparams['mlp_dropout']), nn.ReLU()
				)
...
class MEEGNet(CustomNet):
	...
	def __init__(self, input_size: tuple, n_outputs: int, n_linear: int = 2000, dropout: float = 0.5) -> None:
		super().__init__(input_size, n_outputs)
		...
		lin_size = self.get_lin_size(self.feature_extraction)
		self.classif = nn.Sequential(nn.Linear(lin_size, n_linear // 2), nn.Linear(n_linear // 2, n_outputs))
...
class AutoEncoder(CustomNet):
	def __init__(self, input_size):
		...
		self.encoder = nn.Sequential(nn.Linear(lin_size, 4096), nn.ReLU(True), nn.Linear(4096, 2048))

		self.decoder = nn.Sequential(nn.Linear(2048, 4096), nn.ReLU(True), nn.Linear(4096, lin_size), nn.Tanh())
...
class Model:
	def __init__(
		...
		save_path : str, optional
		    Directory where model outputs (checkpoint .pt, tracker .mat) are saved
	...
	def train(
		...
		assert dataset.data[0].shape == self.input_size, 'Dataset sample size must match network input size.'
...
class TrainingTracker:
	...
	def update(self, epoch, tloss, tacc, vloss, vacc, net, optimizer, early_stop: str = 'loss') -> None:
		...
		check = {'loss': vloss < self.best['validation_loss'], 'accuracy': vacc > self.best['validation_accuracy']}
		if check[early_stop]:
	...
	def plot_metric(self, metric_type: str, option: str = 'both', early_stop: bool = True):
		...
		metric_key = {'accuracy': 'accuracies', 'loss': 'losses'}
