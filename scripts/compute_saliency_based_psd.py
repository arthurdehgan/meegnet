	...
	with disk_semaphore:
		...
		for saliency_type in ('pos', 'neg'):
			save_path = os.path.join(
				args.save_path, 'saliency_based_psd', name, saliency_type + f'_{args.confidence}confidence'
			)
...
if __name__ == '__main__':
	...
	subjects = np.unique(
		[file.split('_')[0] for file in file_list if set(file.split('_')).intersection(set(labels)) != set()]
	)[: args.max_subj]
