...
from meegnet.params import DATA_PATH, SAVE_PATH, SUB_DF
...
def load_freq_data(dataframe, path=DATA_PATH):
	...
	for row in dataframe[: int(len(dataframe))].iterrows():
		...
		sub_data = sp.stats.zscore(extract_bands(np.load(path + '{}_rest_psd.npy'.format(sub))))
		# sub_data = np.load(path + "{}_rest_psd.npy".format(sub))
		# sub_data = sub_data - sub_data.mean(axis=0)[None, :]
		try:
			sub_data2 = sp.stats.zscore(extract_bands(np.load(path + '{}_task_psd.npy'.format(sub))))
...
if __name__ == '__main__':
	...
	for elec in range(N_ELEC):
		...
		randsearch = RandomizedSearchCV(
			SVC(), param_distributions=param_distributions, n_iter=150, cv=cv, random_state=420, n_jobs=-1
		)
		randsearch.fit(X, y)
	...
	# DATA_PATH = path("/home/arthur/data/raw_meegnet/data/data/")
