...
def process_data(arguments):
	...
	while True:
		...
		if len(dataset) == 0:
			continue
		compute_saliency_maps(dataset, labels, sub, sal_path, net, args.confidence, args.clf_type)
...
if __name__ == '__main__':
	...
	csv_file = os.path.join(args.save_path, 'participants_info.csv')
	dataframe = (
	...
	for sub in subj_list:
		t = threading.Thread(target=produce_data, args=(queue, sub, args, disk_semaphore))
		t.start()
		threads.append(t)

	# Start the consumer processes
	with multiprocessing.Pool(processes=NUM_CONSUMERS) as pool:
		pool.map(process_data, [(queue, labels, sal_path, my_model.net, args)] * NUM_CONSUMERS)
