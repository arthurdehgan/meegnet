...
class EpochedDataset:
	...
	def preload(self, data_path: str, csv_path: str = None, subject_col: str = 'sub') -> pd.DataFrame:
		"""
	...
	def _load_csv(self, data_path, csv_name='participants_info.csv') -> pd.DataFrame:
		...
		participants_df = df.sample(frac=1, random_state=self.random_state).reset_index(drop=True)[: self.n_subjects]
