...
import scipy as sp
import numpy as np
from sklearn.model_selection import cross_val_score, RandomizedSearchCV, StratifiedShuffleSplit as SSS, train_test_split
...
def print_info_classif(args):
	...
	if args.clf in ['perceptron', 'SVM', 'RF', 'XGBoost']:
		print('Random search will be used to fine tune hyperparameters.')
...
def load_data(elec_index, args):
	...
	if args.verbose > 0 and args.time:
		start = time.time()
	train_set = load_freq_data(train_df, elec_index, get_features, train_labels, args, data_path)
	test_set = load_freq_data(test_df, elec_index, get_features, test_labels, args, data_path)
	if args.verbose > 0 and args.time:
		print('Time spent loading data:', elapsed_time(time.time(), start))
	if args.verbose > 0:
		print('Done')
		print(f'number of subjects for training: {len(train_df)}, testing: {len(test_df)}')
...
def random_search(args, cv, X, y, groups=None):
	...
	if args.clf == 'RF':
		...
		param_grid = {'penality': ['l2', 'l1', 'elasticnet'], 'alpha': [0.0001, 0.001, 0.00001, 0.0005]}
		randsearch = RandomizedSearchCV(
		...
		param_distributions = {'C': sp.stats.expon(scale=10), 'gamma': sp.stats.expon(scale=0.1)}
		randsearch = RandomizedSearchCV(
			SVC(),
...
def classif(train_set, test_set, args):
	...
	if args.verbose > 0 and args.time:
		start = time.time()
	test = np.mean(cross_val_score(clf, X_test, y_test, groups=groups_test, cv=cv, n_jobs=args.cores))
	if args.verbose > 0 and args.time:
	...
	for elec, elec_name in enumerate(elec_list):
		...
		if not os.path.exists(savename) or args.test:
			...
			if args.test:
				...
				print(f'example groups train: {groups[pattern]}, test: {groups_test[pattern]}')
