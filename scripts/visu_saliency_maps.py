...
if __name__ == '__main__':
		...
		for label in labels:
			...
			for saliency_type in saliency_types:
				...
				saliency_file = os.path.join(
					sal_path, f'{sub}{lab}_{saliency_type}_sal_{args.confidence}confidence.npy'
				)
			...
			if i == random_subject_idx:
				...
				for val in data_dict.values():
					if val.size == 0:
						skip = True
						break
				temp = {key: val[np.random.choice(np.arange(len(val)))] for key, val in data_dict.items()}
	...
	for label in labels:
		for saliency_type in saliency_types:
			if isinstance(all_saliencies[saliency_type][label], list):
