...
if __name__ == '__main__':
	...
	for i, filtr in enumerate(model_weights[0]):
		...
		im, _ = plot_topomap(filtr[0, :, :].ravel(), info, res=128, show=False, contours=1, extrapolate='local')
		plt.axis('off')
	...
	dataframe = (
		pd.read_csv(os.path.join(args.save_path, 'participants_info.csv'), index_col=0)
