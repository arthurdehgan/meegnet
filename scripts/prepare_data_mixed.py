...
def load_and_process(sub_folder: str, sfreq: int, data_path: str, save_path: str, label: str):
		...
		if bads == []:
			if dataset in ('passive', 'smt'):  # dataset != "rest"
				...
				if unique_events == {6, 7, 8, 9}:
					event_dict = {6: 'auditory', 7: 'auditory', 8: 'auditory', 9: 'visual'}
				...
				if len(events) != len(data):
					bad_subj_found(
						sub=sub,
						info='Size missmatch between events and data.',
